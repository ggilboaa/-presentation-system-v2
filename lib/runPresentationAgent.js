...
function getClient() {
  ...
  // maxRetries: 0 — we do our own fallback between models. SDK retries
  //   (default 2 with exponential backoff) would add 10-20s on flaky
  //   requests and blow past Vercel's 60s function limit.
  // timeout: 45s — hard cap per attempt so a slow Anthropic response
  //   can't consume the whole function budget.
