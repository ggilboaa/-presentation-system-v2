...
async function stageContent({ topic, audience, purpose, notes }) {
  ...
  for (const s of json.slides) {
    if (!s.type || !s.title || !Array.isArray(s.content)) {
      throw new Error(`[content] slide missing required fields: ${JSON.stringify(s).slice(0,200)}`);
    }
  }
...
async function stageDesign({ contentOutput, theme }) {
  ...
  for (const s of json.slides) {
    if (!allowed.has(s.layout)) {
      throw new Error(`[design] invalid layout "${s.layout}" on slide "${s.title}". Allowed: A-E.`);
    }
