...
const MODEL_FALLBACK = "claude-3-5-sonnet-latest";
...
// Stage 1 — content (LLM)
// ─────────────────────────────────────────────────────────────
async function stageContent({ topic, audience, purpose, notes }) {
  ...
  const systemParts = [
    ...
    { type: "text", text: "Return ONLY the JSON object specified in your instructions. No prose, no code fences, no commentary." }
  ];

  const input = { topic, audience: audience || null, purpose: purpose || null, notes: notes || null };
  const userMessage = `Generate the presentation for this input:\n\n${JSON.stringify(input, null, 2)}`;

  const { text, model } = await callLLM({ systemParts, userMessage, maxTokens: 4096 });
  const json = extractJson(text, "content");

  if (!Array.isArray(json.slides) || json.slides.length === 0) {
    throw new Error("[content] response missing or empty `slides[]`");
  }
  // Soft repair: the model sometimes omits title or gives content as a string.
  // Only `type` is non-recoverable — everything else we can fill in.
  ...
  json.slides = json.slides.map((s) => {
    if (!s || typeof s !== "object" || !s.type) {
      throw new Error(`[content] slide missing 'type': ${JSON.stringify(s).slice(0,200)}`);
    }
  ...
  console.log(`[content] done — ${json.slides.length} slides, model=${model}`);
  return json; // { coreInsight, slides: [{type,title,content[]}] }
}

// ─────────────────────────────────────────────────────────────
// Stage 2 — design (LLM)
// ─────────────────────────────────────────────────────────────
async function stageDesign({ contentOutput, theme }) {
  console.log("[design] starting");

  const systemParts = [
    { type: "text", text: CONFIG,       cache_control: { type: "ephemeral" } },
    { type: "text", text: SKILL_DESIGN, cache_control: { type: "ephemeral" } },
    { type: "text", text: "Return ONLY the JSON object specified in your instructions. No prose, no code fences, no commentary. Every slide must include layout in {A,B,C,D,E}." }
  ];

  const input = { theme: pickTheme(theme), contentOutput };
  const userMessage = `Apply the design system to this content. Input:\n\n${JSON.stringify(input, null, 2)}`;

  const { text, model } = await callLLM({ systemParts, userMessage, maxTokens: 4096 });
  const json = extractJson(text, "design");

  if (!Array.isArray(json.slides) || json.slides.length === 0) {
    throw new Error("[design] response missing or empty `slides[]`");
  }
  const allowed = new Set(["A", "B", "C", "D", "E"]);
  // Soft repair: opening/closing → A if layout missing. Otherwise default to B.
  for (const s of json.slides) {
    if (!allowed.has(s.layout)) {
      if (s.type === "opening" || s.type === "closing") s.layout = "A";
      else s.layout = "B";
    }
    if (typeof s.content === "string") s.content = [s.content];
    if (!Array.isArray(s.content)) s.content = [];
  ...
  console.log(`[design] done — ${json.slides.length} slides, model=${model}`);
  return json; // { slides: [{type,layout,title,content[],visualHint}] }
}
...
export async function runPresentationAgent({ topic, audience, purpose, notes, theme, origin }) {
  ...
  const contentOutput = await stageContent({ topic, audience, purpose, notes });
  writeTmp("content.json", contentOutput);

  const designOutput  = await stageDesign({ contentOutput, theme });
  writeTmp("design.json", designOutput);
  ...
  const html = buildHtml({ slides: designOutput.slides, theme, topic });
  writeTmp("presentation.html", html);

  const urls = buildExportUrls({ designOutput, theme, topic, origin });
  ...
  return {
    ...
    topic: topic.trim(),
    theme: pickTheme(theme),
    coreInsight: contentOutput.coreInsight || null,
    slideCount: designOutput.slides.length,
